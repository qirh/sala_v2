...
| `src/locales/{en,ar}.json` | `src/locales/{en,ar}.json` | Copy verbatim. **But:** `Home.vue` consumes `p1`–`p5` via vue-i18n's `<i18n>` *component* with a slotted FontAwesome smile in `p2` (replacing `{smile}`). svelte-i18n has no slot-based interpolation. In both locales `{smile}` is the *last token* of `p2` (verified — there's nothing after it in `en.json` or `ar.json`), so split into a single `p2_before_smile` key per locale and render `{$_('p2_before_smile')}<Icon icon="smile" />` in `Home.svelte`. No trailing-text key needed. Leave the original `p2` key in JSON for rollback. |
...
3. **`Home.svelte` `<i18n>` component handling**. In `src/components/Home.vue:16-26`, vue-i18n's `<i18n>` *component* renders `p2` with a slotted FontAwesome smile icon replacing the `{smile}` placeholder. svelte-i18n has no slot interpolation. **`{smile}` is always the last token of `p2`** in both `en.json` (`"...funny accents) {smile}"`) and `ar.json` (`"...لهجتك تضحك {smile}"`) — verified. Translate by:
   - Adding a single `p2_before_smile` key to `src/locales/{en,ar}.json`, containing the literal text up to (but not including) `{smile}`. No trailing-text key needed.
   - In `Home.svelte`, render: `<p>{$_('p2_before_smile')}<Icon icon="smile" /></p>`.
...
| `<i18n>` slot interpolation in `Home.vue` (`p2` with `{smile}`) doesn't have a 1:1 svelte-i18n equivalent. | Step 7 introduces a single `p2_before_smile` key per locale and appends the icon. `{smile}` is the trailing token in both `en.json` and `ar.json`, so no trailing-text key is needed. The original `p2` key stays in JSON for rollback. |
